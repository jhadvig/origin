...
func (r *REST) Get(ctx kapi.Context, name string, opts runtime.Object) (runtime.Object, error) {
	...
	case api.BuildStatusNew, api.BuildStatusPending:
		if buildLogOpts.NoWait {
			glog.V(4).Infof("Build %s/%s is in %s state. No logs are yet available", build.Namespace, name, build.Status)
	...
	case api.BuildStatusCancelled:
		return nil, errors.NewBadRequest(fmt.Sprintf("build %s/%s was cancelled. No logs are available", build.Namespace, build.Name))

	// An error occurred launching the build, return an error
	case api.BuildStatusError:
		return nil, errors.NewBadRequest(fmt.Sprintf("build %s/%s is in an error state. No logs are available", build.Namespace, build.Name))
	}
...
func (r *REST) waitForBuild(ctx kapi.Context, build *api.Build) error {
	...
	go func(ch <-chan watch.Event) {
		for event := range ch {
			...
			switch obj.Status {
			case api.BuildStatusCancelled:
				errchan <- fmt.Errorf("Build %s/%s was cancelled. No logs are available", build.Namespace, build.Name)
				break
			case api.BuildStatusError:
				errchan <- fmt.Errorf("Build %s/%s is in an error state. No logs are available", build.Namespace, build.Name)
				break

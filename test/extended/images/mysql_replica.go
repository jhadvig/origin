...
func CreateMySQLReplicationHelpers(c kclient.PodInterface, masterDeployment, slaveDeployment, helperDeployment string, slaveCount int) (exutil.Database, []exutil.Database, exutil.Database) {
	podNames, err := exutil.WaitForPods(c, exutil.ParseLabelsOrDie(fmt.Sprintf("deployment=%s", masterDeployment)), 1, 60*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	masterPod := podNames[0]

	slavePods, err := exutil.WaitForPods(c, exutil.ParseLabelsOrDie(fmt.Sprintf("deployment=%s", slaveDeployment)), slaveCount, 120*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
	...
	helperNames, err := exutil.WaitForPods(c, exutil.ParseLabelsOrDie(fmt.Sprintf("deployment=%s", helperDeployment)), 1, 60*time.Second)
	o.Expect(err).NotTo(o.HaveOccurred())
...
func replicationTestFactory(oc *exutil.CLI, template string) func() {
	return func() {
		...
		assertReplicationIsWorking := func(masterDeployment, slaveDeployment string, slaveCount int) {
		...
		assertReplicationIsWorking("mysql-master-1", "mysql-slave-1", 1)
